...
## Follow-up — PWA login broke after the console edit

You added `https://orbit-507316.web.app` and then login started returning
`redirect_uri_mismatch`. I probed the client again:
...
**Cause: the edit replaced the redirect URI list instead of appending to
it.** `https://orbit-507316.firebaseapp.com/__/auth/handler` used to be
there — my probe on the previous run recorded it as REGISTERED — and it is
gone now. That handler is the page Firebase Auth's sign-in popup redirects
through, so removing it breaks login on every platform that uses the popup,
which is the PWA.

Nothing in the code caused this and nothing in the code can work around it:
the handler URL is fixed by Firebase Auth's `authDomain`, and there is no
API to edit OAuth client URIs.

### The fix — one line back

In the same OAuth client, **Authorized redirect URIs** must contain both:

    https://orbit-507316.firebaseapp.com/__/auth/handler   <- re-add, was removed
    https://orbit-507316.web.app                           <- keep, for Gmail connect

**Authorized JavaScript origins** keeps `https://orbit-507316.web.app`.
